...
class PhotoRenamer:
    ...
    def __init__(self, root):
        """初始化主窗口及变量"""
        self.root = root
        self.root.title("照片视频批量重命名工具 V1.1")
    ...
    def get_media_datetime(self, filepath):
            ...
            try:
                with open(filepath, 'rb') as f:
                    ...
                    for tag_name in tags.keys():
                        if 'DateTime' in tag_name or 'Date' in tag_name:
                            found_tags.append((tag_name, str(tags[tag_name])))
                    if found_tags:
                        self.log(f"[ExifRead] 在文件中找到以下日期标签: {found_tags}", "INFO")

                    # 尝试标准EXIF日期标签
                    for tag_key, reason in date_tags_to_try:
                        if tag_key in tags:
                            dt_str = str(tags[tag_key])
                            self.log(f"[ExifRead] 找到标签 {tag_key} = {dt_str}", "INFO")
                            try:
                    ...
                    for xmp_tag in xmp_tag_candidates:
                        if xmp_tag in tags:
                            ...
                            dt = self.parse_xmp_content(xmp_str)
                            if dt:
                                self.log(f"[XMP] 从 {xmp_tag} 中解析到原始拍摄时间 {dt}", "INFO")
        ...
        try:
            ...
            if exif:
                ...
                found_pil = [(name, value) for name, value in pil_tags.items()
                             if 'DateTime' in name or 'Date' in name]
                if found_pil:
                    self.log(f"[PIL] 找到以下日期标签: {found_pil}", "INFO")
                ...
                for tag_name, reason in pil_priority:
                    if tag_name in pil_tags:
                        ...
                        self.log(f"[PIL] 找到标签 {tag_name} = {dt_str}", "INFO")
                        try:
                            dt = self.parse_date_string(dt_str)
    ...
    def show_fallback_files_dialog(self, fallback_files, folder, reasons_dict):
        """显示使用回退时间的文件列表对话框（支持双击打开文件夹）"""
        dialog = tk.Toplevel(self.root)
        dialog.title("使用回退时间的文件列表")
        ...
        for f in fallback_files:
            ...
            reason = reasons_dict.get(f, "未知原因")
            listbox.insert(tk.END, f"{rel}  [{reason}]")
    ...
    def start_rename(self):
        ...
        for filepath in media_files:
            ...
            if is_fallback:
                ...
                fallback_reasons[filepath] = reason

        if fallback_files:
            if not self.show_fallback_files_dialog(fallback_files, folder,
                                                    fallback_reasons):
                self.log("用户取消重命名", "INFO")
                return
            self.log(f"有 {len(fallback_files)} 个文件使用回退时间", "INFO")
